...
class AuthProvider with ChangeNotifier {
  ...
  Future<ResponseModel> forgetPassword(ForgetPasswordSendModel forgetPasswordSendModel) async {
    notifyListeners();
    ApiResponse apiResponse = await authRepo.forgetPassword(forgetPasswordSendModel);
    ResponseModel responseModel;
    int? responseCode = apiResponse.response?.statusCode;
    print('code      ${apiResponse.response?.statusCode}');
  ...
  Future<ResponseModel> changePassword(ChangePasswordSendModel changePasswordSendModel) async {
    notifyListeners();
    ApiResponse apiResponse = await authRepo.changePassword(changePasswordSendModel);
    ResponseModel responseModel;
    int? responseCode = apiResponse.response?.statusCode;
    print('code ${apiResponse.response?.statusCode}');
    ...
    if (apiResponse.response != null && apiResponse.response?.statusCode == 200) {
      ...
      if( map['message'] == 'Password not match'){
        responseModel = ResponseModel(false, '${map['message']}',responseCode!);
