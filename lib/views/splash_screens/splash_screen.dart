...
class _SplashScreenState extends State<SplashScreen> {
  ...
  void initState() {
    ...
    Provider.of<LocationProvider>(context, listen: false).getMunicipalitiesAll();
    });

    Timer(const Duration(seconds: 3), () {
      final box = GetStorage();
      String? token = box.read(AppConstants.TOKEN) ?? '';
     // box.write(AppConstants.TOKEN,'59|Zhm6OE6Mj9GVXRiIRbNxm0o3bUumZlSr6yEOv8N3');
      print('sgsdgsd $token');
      //Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const RecentActivity()));
      if(token == ''){
        ...
        // final box = GetStorage();
        // String? password = box.read(AppConstants.USER_PASSWORD);
        // String? email =  box.read(AppConstants.USER_EMAIL);
        // UserLogin userlogIn = UserLogin(email: email ,password: password);
        // Provider.of<AuthProvider>(context,listen: false).login(userlogIn).then((value) {
        //   Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const HomeScreen()));
        // });
      }
    });
          ...
          child: Stack(
            children: [
              Positioned(
                ...
                bottom: 0,
                child: Center(
                    child: SvgPicture.asset(AppImages.ic_logo,width: 227,height: 227,)),
              ),
              Positioned(
                  bottom: 0,
                  left: 0,
                  ...
                  child: Container(
                    ...
                    child: Column(
                      children: [
                        Text(AppConstants.COPY_RIGHT_BOTTOM,
                          textAlign: TextAlign.center,
                          style: TextStyle(color: ColorsResource.TEXT_GRAY_COLOR,fontSize: Dimensions.BODY_10,fontWeight: FontWeight.normal),)
                      ],
                    ),
                  ))
            ],
